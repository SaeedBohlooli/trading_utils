...
def send_market_order_w_sl_tp(ib, side, contract, stop_loss_price, take_profit_price, quantity, order_ref, candle_date=''):
    #current_price = ib_utils.get_current_price_for_contract(ib, contract)
    ...
    logger.info("order sent.. we sleep 3 sec to order get executed ( reset_MAP get executed if we go fastt...)... temp fix..")
    time.sleep(3)
    return data
